...
export default function EditPublication() {
  ...
  useEffect(() => {
    const loadPublication = async () => {
      try {
        const response = await publicationsApi.getById(params.id);
        if (response.success && response.data) {
          ...
          setFormData({
            ...
            type: d.type || '',
            category: d.category || '',
            publication_date: d.publication_date ? new Date(d.publication_date).toISOString().split('T')[0] : '',
            authors: Array.isArray(d.authors) ? d.authors : typeof d.authors === 'string' ? JSON.parse(d.authors || '[]') : [''],
            abstract: d.abstract || '',
        ...
        setErrors({ submit: 'Failed to load publication details.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await publicationsApi.update(params.id, finalData);
      router.push('/admin/publications');
    } catch (error) {
      console.error('Error updating publication:', error);
      setErrors({ submit: error.message || 'Failed to update publication. Please try again.' });
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } finally {
      setSaving(false);
    }
