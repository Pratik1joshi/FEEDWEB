...
export default function EditVideo() {
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        ...
        setErrors({ submit: 'Failed to load video details.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await videosApi.update(params.id, finalData);
      ...
      setErrors({ submit: error.message || 'Failed to update video. Please try again.' });
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } finally {
      setSaving(false);
    }
